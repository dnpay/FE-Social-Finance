...
export const PostCard = memo(({ post, token }: PostCardProps) => {
    const queryClient = useQueryClient();

    const likeMutation = useMutation({
        mutationFn: () => postApi.likePost(post.id, token),
        onSuccess: (data) => {
    ...
    const dislikeMutation = useMutation({
        mutationFn: () => postApi.dislikePost(post.id, token),
        onSuccess: (data) => {
            if (data.action === "removed") {
                message.success({
    ...
    const handleLike = () => {
        if (!token) {
            message.warning({
                content: "⚠️ Vui lòng đăng nhập để like/dislike bài viết",
                duration: 3,
